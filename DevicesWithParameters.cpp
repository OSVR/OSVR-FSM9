...
void createFSM9(VRPNMultiserverData &data, OSVR_PluginRegContext ctx,
                      const char *params) {
    Json::Reader reader;
    Json::Value root;
    if (!reader.parse(params, root)) {
        std::cerr
            << "-- createFSM9(): ERROR: Could not parse configuration: "
	...
	reg.registerDevice(vrpn_Freespace::create(reg.useDecoratedName(data.getName("FSM9")).c_str(),
		reg.getVRPNConnection(), hmd_index));
    reg.setDeviceDescriptor(osvr::util::makeString(com_osvr_FSM9_json));
}
